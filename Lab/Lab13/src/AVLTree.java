...
private void makeAVL(AVLNode node) {
    ...
    // Check if the node is balanced and rotate as necessary
    int tiltDegree = AVLNode.tiltDegree(node);
    if (tiltDegree > 1) {
        // left subtree is too heavy
        if (AVLNode.tiltDegree(node.left) >= 0) {
    ...
    } else if (tiltDegree < -1) {
        // right subtree is too heavy
        if (AVLNode.tiltDegree(node.right) <= 0) {
...
private AVLNode rotateRight(AVLNode node) {
	...
	private boolean isAVL(AVLNode node) {
	    // Check the tilt degree of the current node
	    int tiltDegree = AVLNode.getHeight(node.left) - AVLNode.getHeight(node.right);
	    if (tiltDegree < -1 || tiltDegree > 1) {
	        return false;
	    }

	    // Recursively check the left and right subtrees
	    if (node.left != null && !isAVL(node.left)) {
	        return false;
	    }
	    if (node.right != null && !isAVL(node.right)) {
	        return false;
	    }
	...
	private static boolean same(AVLNode node1, AVLNode node2) {
