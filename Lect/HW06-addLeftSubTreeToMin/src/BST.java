...
public class BST {
	...
	public void addLeftSubTreeToMin(BST subtree) {
		...
		BSTNode min = ((TreeIterator)findMin()).currentNode;
		
		TreeIterator itr = (TreeIterator)subtree.findMax();
		int maxSubtree = itr.currentNode.data;

		if (maxSubtree > min.data) {
			return;
		}
		
		subtree.root.parent = min;
		min.left = subtree.root;
		size += subtree.size;
