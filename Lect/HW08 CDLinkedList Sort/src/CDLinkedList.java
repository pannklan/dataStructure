	...
	public void sort() throws Exception { // any sorting will do BUT you must use only 'this' list. No array or any other data structures allowed!!!
		...
		if (isEmpty() || size == 1) {
		 ...
		 while (swapped) {
		     ...
		     while (itr.hasNext()) {
		    	DListNode currentNode = itr.currentNode;
		        DListNode nextNode = currentNode.nextNode;
		        if (nextNode == header) {
		        	 break;
		        }
...
//	    if (isEmpty()) return;
//
//	    DListNode cur = header.nextNode;
//	    while (cur.nextNode != header) {
//	        DListNode min = cur;
//	        DListNode innerCur = cur.nextNode;
//	        while (innerCur != header) {
//	            if (innerCur.data < min.data) {
//	                min = innerCur;
//	            }
//	            innerCur = innerCur.nextNode;
//	        }
//	        if (min != cur) {
//	            // swap data between min and cur nodes
//	            int temp = min.data;
//	            min.data = cur.data;
//	            cur.data = temp;
//	        }
